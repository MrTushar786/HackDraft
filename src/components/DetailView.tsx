...
${idea.timeline?.map(p => `[${p.phase} — ${p.duration}]\n${p.tasks?.map(t => `  - ${t}`)?.join('\n') ?? ''}`)?.join('\n\n') ?? ''}
  ...
  const downloadWorkspace = async () => {
    ...
    // Default Configs
    zip.file(".env.example", "# Generated by HackMind AI\nDATABASE_URL=\nAPI_KEY=\nNEXT_PUBLIC_API_URL=\n");
  ...
  const downloadManualPDF = () => {
    ...
    doc.setFont('helvetica', 'bold');
    doc.text('HACKMIND AI ENGINE V2.0', margin, 15);
    ...
    for (let i = 1; i <= finalPageCount; i++) {
       ...
       doc.text('HACKMIND AI BLUEPRINT ENGINE • CONFIDENTIAL STRATEGY DOCUMENT', margin, pageHeight - 8);
    }

    // Final Download
    doc.save(`HackMind-COMPLETE-Blueprint-${idea.name.replace(/\s+/g, '-')}.pdf`);
  };
